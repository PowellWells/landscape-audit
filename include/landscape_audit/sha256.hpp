...
inline std::string sha256(std::span<const std::uint8_t> input) {
    ...
    for (std::size_t offset = 0; offset < padded.size(); offset += 64U) {
        ...
        for (std::size_t i = 16; i < words.size(); ++i) {
            const auto s0 = rotate_right(words[i - 15], 7U) ^ rotate_right(words[i - 15], 18U) ^ (words[i - 15] >> 3U);
            const auto s1 = rotate_right(words[i - 2], 17U) ^ rotate_right(words[i - 2], 19U) ^ (words[i - 2] >> 10U);
